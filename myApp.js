...
const createAndSavePerson = function(done) {
  let luciano = new Person({
    name:"Luciano Galusso", 
    age: 24, 
    favoriteFoods: ["pasta", "pizza", "burger"]
  });

  luciano.save(function(err, data) {
    if (err)
...
const removeManyPeople = (done) => {
  ...
  Person.remove({name: nameToRemove}, null, (err, updatedDoc) => {
    if(err) 
      return console.log(err);
    done(null, updatedDoc);
  });
